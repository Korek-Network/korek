...
test("creates a signed transfer",()=>{const made=createEncryptedWallet("long test password");const wallet=unlockWallet(made.file,"long test password");const tx=signTransfer(wallet,made.file.address,"2.5");assert.equal(tx.amount,"250000000");assert.ok(tx.signature)});
